shiratSite.controller('library', ['$rootScope', '$scope', '$state','$timeout', function ($rootScope, $scope, $state,$timeout) {
   ...
   $scope.getAllLessons = function(){
	   $.ajax({
		  url:  $scope.domain+'?json=get_recent_posts&post_type=audio',
		...
		$.ajax({
		  url:  $scope.domain+'?json=get_recent_posts&post_type=video',
   ...
   $scope.getAllLessons()
    $scope.lessonClick =function(lessonData){
	   $state.transitionTo('lesson',{lesID:lessonData.id})
   }
} ]);
